...
- **Dual-bucket rate limiting**, per anonymous patron id *and* per IP (a sliding window on each), so no single client or single host can issue searches at an unbounded rate. To be precise about its scope: this limiter is a per-instance burst control, not a platform-wide daily bound — that bound is provided by the daily spend counter described immediately below, which is now shipped.
- **A cross-instance daily `search.list` spend counter, enforced platform-wide** (`lib/search-budget.ts`). Every outbound `search.list` call reserves against one shared counter before it is issued, so the platform-wide daily total is bounded no matter how many serverless instances are running. The counter stops at **90 calls**, a deliberate safety margin of 10 below the 100/day cap, so the allowance can never be silently drained. It is keyed on the calendar date in **America/Los_Angeles**, matching Google's own midnight-Pacific quota reset rather than UTC (a UTC key would roll the day 7–8 hours early). The reservation is atomic, and it **fails closed**: if the shared counter is unreachable, searches are refused rather than spent unaccounted. Paste-a-link is unaffected in every one of these cases, so the product keeps working when search is bounded.
- **Paste-a-link is a first-class path** that bypasses search entirely and costs no `search.list` quota.

**Planned, not started:** a locally-harvested karaoke song index, populated via `playlistItems.list` from the separate 10,000-unit pool, so that most searches are answered locally and `search.list` becomes a long-tail fallback rather than the core interaction. We mention the index explicitly rather than omit it: it would be an internal implementation detail behind our own search box, never a publicly browsable catalogue.
...
- **Data storage — queue contents.** When a guest adds a song, we store the `videoId` and title in that room's queue so the TV can play it and the room can see what is coming. **These entries currently have no automatic expiry**; they persist until the entry is played and removed, or the room is deleted. We are treating that as an item to fix rather than to describe loosely: we intend to add an explicit retention bound to room/queue data. To be exact about where that work stands as of 2026-09-01: a configurable retention window now exists in the code and is applied to the **room metadata** record, but it ships disabled by default, no window has been chosen yet, and it does **not** yet cover the queue entries described in this paragraph — so the sentence above stands as written. Separately, our anonymous product-analytics events already carry a hard 90-day expiry.
...
- [x] ~~Update §2's "in progress" line if TICKET-87 (daily spend counter) has merged by filing time.~~ Done: TICKET-87 merged as PR #65 (2026-08-20) and §2 now describes the shipped counter — 90-call budget under the 100/day cap, Pacific-day key, atomic, fail-closed — as a shipped bound rather than as work in progress.
