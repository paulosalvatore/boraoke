...
**Fix (`app/(patron)/[room]/PatronRoom.tsx`, in this ticket's file scope).** The header can now absorb its own content instead of shoving the trailing control off screen: `flex-wrap: wrap` + a `gap` on the header, `min-width: 0` + `flex-shrink: 1` + `overflow: hidden` on the greeting group, `text-overflow: ellipsis` on the nickname button, and `flex-shrink: 0` on the language switcher and the "Oi," label so they never collapse. Plus `margin-left: auto` on `.headerTrigger`, so that when the row does wrap the button lands flush right rather than orphaned at the far left.

Measured outcome (App Tester re-test, `_measurements-retest.json`): at both 390 and 320 with an 18-char nickname the trigger sits fully inside the viewport (right edge 374/390 and 304/320), header overflow is **0** at both widths, `position` is still `static`, and there is no queue-row overlap at any of the five scroll fractions. Wrapping is what actually resolves it in practice — the header becomes two rows and the nickname stays fully readable; the ellipsis is defence-in-depth that does not need to fire at these widths. A 24-char stress nickname at 320px wraps the trigger onto its own right-aligned row, still fully visible and unclipped.
