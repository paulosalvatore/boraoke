...
export default async function RoomTvPage({
  ...
  //
  // TICKET-79: `<html lang>` now agrees with these messages in the SERVER-
  // RENDERED response — `i18n/request.ts` applies the TV route's own room-locale
  // chain, so the root layout emits the right attribute from the start. The
  // TICKET-75 inline `document.documentElement.lang = …` script that used to sit
  // in this subtree is therefore gone: it patched the DOM after hydration, which
  // left the served HTML (JS disabled, crawlers that do not execute scripts)
  // still carrying the visitor's cookie locale. This provider stays — it is what
  // keeps the MESSAGES room-scoped, and it makes the lang/content agreement
  // hold even if the pathname header never reaches the request config.
