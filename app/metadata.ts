...
// ─── Per-locale metadata (TICKET-30 i18n wave) ───────────────────────────────
 ...
 * static `metadata` export when both exist; layout re-exports THIS. Title,
 * description, and the OG image/locale all follow the request locale (cookie /
 * Accept-Language via i18n/request.ts), with pt-BR as the fallback for both copy
 * and image. The static `metadata` above is retained as the pt-BR baseline (and
 * its unit test).
 */
