/**
 * TICKET-31 — GET /api/admin/analytics route tests: the auth gate (reuses the
 * `default`-room host session, same mechanism as every other host route) and
 * basic request-shape validation. The aggregation math itself is covered by
...
function req(opts: { authed?: boolean; qs?: string } = {}): NextRequest {
  ...
  return new NextRequest(
    `http://127.0.0.1:3040/api/admin/analytics${opts.qs ? `?${opts.qs}` : ""}`,
    { headers },
  );
}
...
describe("GET /api/admin/analytics — auth gate", () => {
  ...
  it("401s with a garbage cookie value", async () => {
    const res = await GET(
      new NextRequest("http://127.0.0.1:3040/api/admin/analytics", {
...
describe("GET /api/admin/analytics — request validation (read-only, no mutation)", () => {
